...
<head>
    ...
    <title>Registrasi - Pelajarnuju</title>
    ...
    <div class="md:w-7/12 flex items-center justify-center p-6 md:py-10 md:px-12 relative w-full">
        ...
        <div class="w-full max-w-lg bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 mt-8 md:mt-0">
            ...
            <div class="mb-6 text-center md:text-left">
                ...
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Pendaftaran Akun Baru</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Lengkapi data diri Anda di bawah ini dengan benar.</p>
            </div>
            ...
            <form action="{{ route('register') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" placeholder="Nama Lengkap" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#083C30] focus:border-transparent outline-none transition-all text-sm" value="{{ old('name') }}" required>
                ...
                <div>
                    ...
                    <span id="email-msg" class="text-xs text-green-600 font-semibold hidden mt-1"></span>
                    @error('email') <span class="text-red-500 text-xs italic mt-1 block">{{ $message }}</span> @enderror
                </div>

                <!-- Input OTP (Tersembunyi secara default) -->
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="jenis_kelamin" id="jenis_kelamin" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#083C30] focus:border-transparent outline-none transition-all text-sm" required>
                        ...
                        @error('jenis_kelamin') <span class="text-red-500 text-xs italic mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <!-- Nomor Ponsel -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nomor Ponsel</label>
                        <input type="text" name="nomor_ponsel" placeholder="08xxxxxxxxxx" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#083C30] focus:border-transparent outline-none transition-all text-sm" value="{{ old('nomor_ponsel') }}" required>
                        @error('nomor_ponsel') <span class="text-red-500 text-xs italic mt-1 block">{{ $message }}</span> @enderror
                    </div>
                </div>

                <!-- AREA PASSWORD dengan Validasi Real-time -->
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Password</label>
                        <div class="relative">
                            <input type="password" name="password" id="password" placeholder="••••••••" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#083C30] focus:border-transparent outline-none transition-all text-sm pr-12" required>
                        ...
                        <!-- Indikator Password -->
                        <ul class="text-xs font-semibold space-y-1 mt-2">
                    ...
                    @error('password') <span class="text-red-500 text-xs italic block">{{ $message }}</span> @enderror
                </div>

                <button type="submit" id="btn-submit-form" class="w-full mt-2 py-3 px-4 bg-[#083C30] hover:bg-[#062d24] text-white font-semibold rounded-xl shadow-lg shadow-[#083C30]/20 transition-all duration-200 text-sm flex items-center justify-center gap-2">
                    Daftar Sekarang
                </button>
            ...
            <p class="text-center text-sm text-gray-600 dark:text-gray-400 mt-5">
                Sudah punya akun? 
                <a href="{{ route('login') }}" class="text-[#083C30] dark:text-[#3BD59C] font-semibold hover:underline">Login Sekarang!</a>
            </p>
...
<script>
    ...
    btnSendOtp.addEventListener('click', function() {
        ...
        .then(data => {
            if(data.success) {
                ...
                btnSendOtp.innerText = "Kirim Ulang OTP";
            } else {
                emailMsg.innerText = data.message || 'Email tidak valid atau sudah terdaftar.';
        ...
        .catch(error => {
            alert('Terjadi kesalahan pada server. Pastikan email unik dan koneksi stabil.');
            btnSendOtp.innerText = "Kirim OTP";
        })
        .finally(() => {
    ...
    btnVerifyOtp.addEventListener('click', function() {
        ...
        .then(data => {
            if(data.success) {
                ...
                emailMsg.innerText = "✓ Email Terverifikasi!";
                emailMsg.classList.remove('hidden', 'text-red-500');
                emailMsg.classList.add('text-green-600');
            } else {
        ...
        .finally(() => {
            if(!emailInput.readOnly) btnVerifyOtp.innerText = "Cek OTP";
        });
    });

    // 2. SCRIPT TOGGLE MATA PASSWORD
    ...
    // 3. SCRIPT VALIDASI PASSWORD REAL-TIME
