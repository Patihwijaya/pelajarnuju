<x-layouts.app-user title="Tulis Artikel Baru - Pelajarnuju">
    <div class="max-w-5xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Tulis Artikel Baru</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Bagikan ide, opini, atau liputan kegiatan. Tulisan Anda akan direview oleh Redaksi sebelum dipublish.</p>
        </div>
            ...
            <form action="{{ route('user.artikel.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="judul" id="judul" value="{{ old('judul') }}" placeholder="Masukkan judul yang menarik..." class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#083C30] focus:border-[#083C30] block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="penulis" id="penulis" value="{{ Auth::user()->name ?? '' }}" readonly class="bg-gray-100 border border-gray-300 text-gray-600 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-400 cursor-not-allowed select-none">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="kategori" id="kategori" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#083C30] focus:border-[#083C30] block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                            <option value="" disabled {{ old('kategori') ? '' : 'selected' }}>-- Pilih Kategori --</option>
                            <option value="Berita" {{ old('kategori') == 'Berita' ? 'selected' : '' }}>Berita / Liputan</option>
                            <option value="Opini" {{ old('kategori') == 'Opini' ? 'selected' : '' }}>Opini / Esai</option>
                            <option value="Sastra" {{ old('kategori') == 'Sastra' ? 'selected' : '' }}>Sastra (Puisi/Cerpen)</option>
                            <option value="Keislaman" {{ old('kategori') == 'Keislaman' ? 'selected' : '' }}>Kajian Keislaman</option>
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label for="gambar" class="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 relative overflow-hidden transition-colors">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6" x-show="!imagePreview">
                                ...
                                <p class="mb-2 text-sm text-gray-500 dark:text-gray-400"><span class="font-semibold">Klik untuk mengunggah</span></p>
                            </div>
                ...
                <div>
                    ...
                    <textarea id="isi" name="isi" rows="15" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300">{{ old('isi') }}</textarea>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <button type="submit" class="text-white bg-[#083C30] hover:bg-[#062c23] font-medium rounded-lg text-sm px-5 py-2.5 shadow-md">Kirim ke Redaksi</button>
                </div>
            </form>
        </div>
    </div>
