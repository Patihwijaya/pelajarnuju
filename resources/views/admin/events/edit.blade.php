<x-layouts.admin>
    <div class="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8" x-data="eventBuilder()">
        ...
        <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 tracking-tight">Edit Event: {{ Str::limit($event->title, 40) }}</h1>
                <p class="text-sm text-gray-500 mt-1">Perbarui informasi kegiatan, kategori, jadwal, pembayaran, halaman sukses, dan struktur formulir pendaftarannya.</p>
            </div>
        ...
        <!-- Form Update dengan method PUT dan enctype untuk upload file -->
        <form action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="p-6 sm:p-8 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $event->title) }}" required placeholder="Contoh: LDK Angkatan 2026" 
                        ...
                        <div>
                            ...
                            <select name="category" required class="w-full bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow">
                                ...
                                <option value="Kaderisasi" {{ old('category', $event->category) == 'Kaderisasi' ? 'selected' : '' }}>Kaderisasi</option>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="max_participants" value="{{ old('max_participants', $event->max_participants) }}" required placeholder="Contoh: 50" 
                                class="w-full bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow">
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                @if($event->thumbnail)
                                    <div class="shrink-0">
                                        <img src="{{ asset('storage/' . $event->thumbnail) }}" alt="Thumbnail" class="w-16 h-12 object-cover rounded border border-gray-200">
                                    </div>
                                @endif
                                <div class="w-full">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="end_date" value="{{ old('end_date', $event->end_date ? $event->end_date->format('Y-m-d\TH:i') : '') }}" required 
                                class="w-full bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                        <div>
                            ...
                            <input type="datetime-local" name="event_start_date" value="{{ old('event_start_date', $event->event_start_date ? $event->event_start_date->format('Y-m-d\TH:i') : '') }}" 
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="event_end_date" value="{{ old('event_end_date', $event->event_end_date ? $event->event_end_date->format('Y-m-d\TH:i') : '') }}" 
                                class="w-full bg-white border border-gray-300 rounded-lg px-4 py-2 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        </div>
                    </div>

                    <!-- OPSI EVENT BERBAYAR (HTM) -->
                    ...
                    <div>
                        ...
                        <input type="text" name="location" value="{{ old('location', $event->location) }}" required placeholder="Contoh: Aula Utama PCNU Jakarta Utara / Link Zoom" 
                            class="w-full bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="5" required placeholder="Jelaskan detail kegiatan, fasilitas, dan ketentuan di sini..." 
                            class="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow">{{ old('description', $event->description) }}</textarea>
                    </div>
                </div>
            </div>

            <!-- CARD 2: PENGATURAN HALAMAN SUKSES (FEEDBACK) -->
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="bg-gray-50 px-6 py-4 border-b border-gray-200">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Sesuaikan kembali kolom pertanyaan untuk peserta acara ini.</p>
                    </div>
            ...
            <div class="flex justify-end pt-4 space-x-4">
                ...
                <button type="submit" class="px-8 py-2.5 bg-blue-600 rounded-lg text-sm font-bold text-white hover:bg-blue-700 shadow-md">Simpan Perubahan</button>
            </div>
        </form>
    </div>

    <!-- Script Alpine.js untuk Dynamic Form Builder, Payment Accounts, & Settings -->
    <script>
        function eventBuilder() {
            return {
                isPaid: @json(!empty($event->payment_info)),
                paymentAccounts: @json(old('payment_info', $event->payment_info ?? [])),
                fields: @json(old('form_schema', $event->form_schema ?? [])),
