<x-layouts.admin title="Review Artikel - Admin">
    <div class="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        <!-- Tombol Kembali -->
        <div class="mb-6">
            <a href="{{ route('admin.artikel.index') }}" class="text-[#083C30] dark:text-[#3BD59C] hover:underline flex items-center gap-1 text-sm font-medium transition-colors w-fit">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-6 md:p-8 border border-gray-100 dark:border-gray-700">
            ...
            <!-- Bagian Footer / Tombol Aksi di show.blade.php -->
            <div class="flex flex-col md:flex-row items-center justify-between mt-10 border-t pt-5">
                <a href="{{ route('admin.artikel.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium px-5 py-2.5 rounded-lg transition-colors mb-4 md:mb-0">
                    Kembali ke Tabel
                </a>

                <div class="flex flex-col md:flex-row items-center gap-4">
                    @if($artikel->status === 'pending')
                        <span class="text-yellow-600 flex items-center gap-2 font-medium text-sm">
                            <svg class="w-5 h-5 animate-spin text-yellow-500" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                        ...
                        <div class="flex gap-2">
                            <!-- Tombol Tolak -->
                            <form action="{{ route('admin.artikel.reject', $artikel->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menolak artikel ini?');">
                                @csrf
                                <button type="submit" class="bg-red-500 hover:bg-red-600 text-white font-medium px-4 py-2.5 rounded-lg transition-colors shadow">
                                    Tolak
                                </button>
                            </form>
                            <!-- Tombol Setujui -->
                            <form action="{{ route('admin.artikel.approve', $artikel->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-medium px-4 py-2.5 rounded-lg transition-colors shadow">
                                    Setujui & Terbitkan
                                </button>
                            </form>
                        </div>
                    ...
                    @elseif($artikel->status === 'published')
                        <span class="text-green-600 flex items-center gap-2 font-medium text-sm">
                        ...
                        <!-- Tombol Sembunyikan -->
                        <form action="{{ route('admin.artikel.hide', $artikel->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menyembunyikan artikel ini dari publik?');">
                            @csrf
                            <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 text-white font-medium px-4 py-2.5 rounded-lg transition-colors shadow">
                                Sembunyikan Artikel
                            </button>
                        </form>

                    @elseif($artikel->status === 'hidden')
                        <span class="text-gray-500 flex items-center gap-2 font-medium text-sm">
                            <svg class="w-5 h-5 animate-pulse text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"></path></svg>
                            Sedang disembunyikan.
                        </span>
                        <!-- Tombol Tampilkan Kembali (Menggunakan route approve agar status jadi published) -->
                        <form action="{{ route('admin.artikel.approve', $artikel->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-medium px-4 py-2.5 rounded-lg transition-colors shadow">
                                Tampilkan Kembali
                    ...
                    @elseif($artikel->status === 'rejected')
                        <span class="text-red-500 flex items-center gap-2 font-medium text-sm">
                            <svg class="w-5 h-5 animate-pulse text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                            Artikel ini telah ditolak.
                        </span>
                        <!-- Tombol Setujui Ulang (Berjaga-jaga jika Admin berubah pikiran) -->
                        <form action="{{ route('admin.artikel.approve', $artikel->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menyetujui ulang artikel yang sudah ditolak ini?');">
                            @csrf
                            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-medium px-4 py-2.5 rounded-lg transition-colors shadow">
                                Setujui & Terbitkan Ulang
                            </button>
                        </form>
                    @endif
                </div>
