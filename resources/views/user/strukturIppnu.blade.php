<x-layouts.app title="strukturIppnu">
<div>
    <h1 class="font-bold text-2xl">Susunan Pengurus PC IPPNU Jakarta Utara</h1>

    <div class="mt-5 overflow-hidden flex flex-col md:flex md:flex-wrap md:flex-row gap-5 mx-5 md:mx-0 justify-center">
         @foreach($strukturIppnu as $struktur)
            @php
            ...
            <div class="relative w-60 overflow-hidden" x-data="{ open:false }" @click.outside="open = false">
                    @if($struktur->gambar)
                    <img src="{{ asset('uploads/strukturIppnu/'.$struktur->gambar) }}" alt="{{ $struktur->judul }}" class="w-60 object-cover cursor-pointer" @click="open = !open">
                    @endif
            <div
                x-show="open"
                x-transition:enter="transition transform duration-500"
                x-transition:enter-start="translate-y-full opacity-0"
                x-transition:enter-end="translate-y-0 opacity-80"
                x-transition:leave="transition transform duration-500"
                x-transition:leave-start="translate-y-0 opacity-80"
